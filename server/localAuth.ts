...
export async function setupAuth(app: Express) {
  ...
  // Enhanced company registration endpoint
  app.post("/api/auth/register/company", async (req, res) => {
    try {
      const {
        ...
        // Company info
        legalName, tradeName, industry, websiteUrl, companySize, yearFounded, description, logoUrl,
        // Contact info
        contactName, contactJobTitle, phoneNumber, businessAddress,
        // Verification
        verificationMethod, verificationDocumentUrl, einTaxId,
        websiteVerificationMethod, websiteVerificationToken, socialMediaProfiles
      } = req.body;
      ...
      if (!legalName || !industry || !websiteUrl) {
        return res.status(400).json({ error: "Company legal name, industry, and website are required" });
      }

      if (!contactName || !contactJobTitle || !phoneNumber || !businessAddress) {
        return res.status(400).json({ error: "All contact information fields are required" });
      }

      // Validate verification documents
      if (verificationMethod === 'business_registration' && !verificationDocumentUrl) {
        return res.status(400).json({ error: "Business registration document is required" });
      }

      if (verificationMethod === 'ein_tax_id' && !einTaxId) {
        return res.status(400).json({ error: "EIN/Tax ID is required" });
      }
      ...
      await storage.createCompanyProfile({
        ...
        einTaxId: einTaxId || null,
        websiteVerificationMethod: websiteVerificationMethod || null,
        websiteVerificationToken: websiteVerificationToken || null,
      ...
      // Send confirmation email (TODO: implement email verification)
      ...
      req.login(user, (err) => {
        ...
        res.json({
          ...
          message: "Registration submitted successfully. Your application will be reviewed within 24-48 hours."
        });
      });
    } catch (error: any) {
